...
#
# User-facing commands (called from Fish wrappers):
#   gssh-add       Register a profile and generate an SSH key
#   gssh-list      List all registered profiles
#
# Exported helpers (called by Fish via gssh.nu bridge):
#   fetch-profile  Look up a profile by username
#   make-ssh-cmd   Build an SSH command string for a given key
...
export def fetch-profile [username: string] {
    if not ($PROFILES_PATH | path exists) {
        error make {msg: $"No profiles file found at ($PROFILES_PATH). Run gssh-add first."}
    }
    let raw = (open --raw $PROFILES_PATH | str trim)
    if ($raw | is-empty) {
        error make {msg: $"Profiles file is empty. Run gssh-add first."}
    }
